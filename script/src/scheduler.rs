    ...
    DL: CellDataProvider + HeaderProvider + ExtensionProvider + Send + Sync + Clone + 'static,
{
    ...
    fn iterate(&mut self, pause: Pause, limit_cycles: Cycle) -> Result<Cycle, Error> {
        ...
        // log::debug!("Fds and owners: {:?}", self.fds);
    ...
    fn process_message_box(&mut self) -> Result<(), Error> {
        ...
        for message in messages {
            match message {
                ...
                Message::Fd(vm_id, args) => {
                    if self.next_fd_slot - FIRST_FD_SLOT >= MAX_FD {
                        let (_, machine) = self.instantiated.get_mut(&vm_id).unwrap();
                        machine.machine.set_register(A0, MAX_FD_CREATED as u64);
                        continue;
                    }
                    let (p1, p2, slot) = FdId::create(self.next_fd_slot);
                    self.next_fd_slot = slot;
                    // log::debug!("VM {} creates fds ({}, {})", vm_id, p1.0, p2.0);
    ...
    fn process_io(&mut self) -> Result<(), Error> {
        let mut reads: HashMap<FdId, (VmId, VmState)> = HashMap::default();
